...
Copyright Ken Perlin, version by Ashwin Ganapathiraju
...
const p = new Uint8ClampedArray(512);
const permutation = new Uint8ClampedArray([151, 160, 137, 91, 90, 15, // Perlin original values
...
function seed(s) {
  for (let i = 0; i < 256; i++) {
    permutation[i] = Math.floor((permutation[i] * s) % 255);
...
function grad(hash, x, y, z) {
  ...
  const u = h < 8 ? x : y;
  let v;
  if (h < 4) { // I hate you can't nest ternaries
    v = y;
  } else {
    v = h === 12 || h === 14 ? x : z;
  }
...
function noise(i, j, k) {
  const X = Math.floor(i) & 255;
  const Y = Math.floor(j) & 255;
  const Z = Math.floor(k) & 255;
  const x = i - Math.floor(i);
  const y = j - Math.floor(j);
  const z = k - Math.floor(k);
  const u = fade(x);
  const v = fade(y);
  const w = fade(z);
